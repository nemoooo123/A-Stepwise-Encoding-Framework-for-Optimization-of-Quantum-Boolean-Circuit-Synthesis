...
def main():
    ...
    if len(sys.argv) > 1:
        try:
            algo_choice = int(sys.argv[1])
            # 注意：如果是從 sys.argv 傳進來，通常是「題號」而非「完整路徑」
            file_name = sys.argv[2]
        except ValueError:
            print("Error: Command line arguments must be numeric.")
            return
    else:
        # 如果沒有帶參數，保留原本的手動輸入模式（方便手動測試）
        loader.get_info()
        try:
            algo_choice = int(input("Select Algorithm (1: AE-QTS, 2: DE, 3: PSO, 4: TS, 5: QTS, 6: Other): "))
            file_name = input("請輸入任務檔案名稱（例如 tasks_part_1.txt）：")
        except ValueError:
            print("Invalid input.")
            return
    ...
    with open(file_name, 'r', encoding='utf-8') as file:
        ...
        for line in file:
            ...
            if not line:        # 略過空行
                continue
            target_output = [int(x) for x in line.split(',')]
            ...
            for r in range(num_experiments):
                ...
                # Progress reporting
                if (r+1)%10==0:
                    print(f"---Question:{ttttt} | Experiment {r+1} Finished  ---")
        ...
        # --- Automatic Directory and Filename Creation ---
            ...
            base_filename = f"{num_bits}_{numbers}"
